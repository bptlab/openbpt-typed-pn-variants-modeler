...
export default class ModdleCopy {
  ...
  copyElement(sourceElement, targetElement, propertyNames) {
    ...
    propertyNames =
      propertyNames || getPropertyNames(sourceElement.$descriptor);
  ...
  copyProperty(property, parent, propertyName) {
    ...
    const propertyDescriptor = this._moddle.getPropertyDescriptor(
      parent,
      propertyName,
    );
    ...
    if (propertyDescriptor.isId || propertyDescriptor.isReference) {
      if (
        Object.keys(REFERENCES_TO_COPY).includes(parent.$type) &&
        propertyName === REFERENCES_TO_COPY[parent.$type]
      ) {
        return property;
      } else {
        return;
    ...
    if (isArray(property)) {
      return reduce(
        ...
        function (childProperties, childProperty) {
          // recursion
          copiedProperty = self.copyProperty(
            childProperty,
            parent,
            propertyName,
          );
...
ModdleCopy.$inject = ["eventBus", "customElementFactory", "moddle"];
