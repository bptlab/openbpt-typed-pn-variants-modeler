...
export function nextPosition(modeler, type) {
  const existingStates = modeler
    .get("elementRegistry")
    .filter((element) => is(element, type));
  const rightBorder = Math.max(
    ...existingStates.map((element) => element.x + (element.width * 3) / 2),
  );
  const topBorder = Math.min(
    ...existingStates.map((element) => element.y + element.height / 2),
  );
...
export function ifModelElement(fn) {
