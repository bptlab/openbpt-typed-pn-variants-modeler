...
import { getBusinessObject, getDi, is } from "../util/Util";
...
export default class UpdatePropertiesHandler {
  constructor(elementRegistry, moddle, modeling, textRenderer) {
  ...
  execute(context) {
    ...
    const oldProperties =
      context.oldProperties || getProperties(element, properties);
  ...
  postExecute(context) {
    ...
    if (is(element, `${MODELER_PREFIX}:Arc`)) {
      text = getArcInscriptionText(getBusinessObject(element));
    } else {
      text = label && getBusinessObject(label).name;
    }
