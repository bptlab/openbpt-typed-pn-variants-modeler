...
import { createDefaultInscriptionElementForClass, getArcColor } from "../declarations-panel/provider/properties/Util";
...
export default class CustomUpdater extends CommandInterceptor {
  constructor(eventBus, customElementFactory, connectionDocking, translate) {
    ...
    // Update label bounds
    eventBus.on(Events.SHAPE_CHANGED, function (e) {
    ...
    function createArcInscription(e) {
      self.createArcInscription(e.context.connection);
    }

    this.executed([
      'connection.create',
    ], ifModelElement(createArcInscription))
  ...
  createArcInscription = function (arc) {
    if (!arc.businessObject.inscription) {
      const newInscription = this._customElementFactory.create(
        `${MODELER_PREFIX}:Inscription`,
        { inscriptionElements: [] }
      );
      newInscription.$parent = arc;

      const arcColor = getArcColor(arc);
      if (arcColor && arcColor.length) {
        arcColor.forEach((dataClass) => {
          const inscriptionElement = createDefaultInscriptionElementForClass(
            dataClass, newInscription, this._customElementFactory
          );
          newInscription.inscriptionElements.push(inscriptionElement);
        })
      }
      arc.businessObject.inscription = newInscription;
    }
  }
}
