...
export default class CustomVerification extends CommandInterceptor {
	...
	verifyStructuralCorrectness = function (self, transition) {
		...
		if (hasUnbound) {
			let title = "This transition can structurally never be fired.\n\n";
			if (unboundVars.length > 0) {
				...
				title += "Please add at least one data class to all Places.";
			}
			const overlayId = self._overlays.add(transition, {
				...
				html: `
					<div class="unbound-output-overlay" title="${title}" style="cursor: pointer; overflow: visible; font-size: 1em;">
