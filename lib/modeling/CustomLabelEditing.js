...
export default class CustomLabelEditing {
  constructor(eventBus, canvas, directEditing, commandStack, textRenderer) {
    ...
    eventBus.on(Events.ELEMENT_DBLCLICK, function (event) {
      directEditing.activate(event.element);
    });

    // complete on followup canvas operation
    eventBus.on([
      Events.AUTO_PLACE_START,
      Events.CANVAS_VIEWBOX_CHANGING,
      Events.DRAG_INIT,
      Events.ELEMENT_MOUSEDOWN,
      Events.POPUP_MENU_OPEN
    ], function (event) {
    ...
    eventBus.on(Events.DIRECT_EDITING_ACTIVATE, function (context) {
      if (is(context.active.element, `${MODELER_PREFIX}:Arc`)) {
        directEditing.cancel();
      }
    });

    // cancel on command stack changes (= when some other action is done)
    eventBus.on(Events.COMMANDSTACK_CHANGED, function () {
      if (directEditing.isActive()) {
        directEditing.cancel();
      }
    });

    eventBus.on(Events.CREATE_END, 500, function (event) {
    ...
    eventBus.on(Events.AUTO_PLACE_END, 500, function (event) {
