...
export function getArcInscriptionText(arcBusinessObject) {
  ...
  const inscriptionText = inscription.inscriptionElements
    .map((inscriptionElement) => {
      ...
      const variableTypeSuffix =
        variableType && variableType.id === inscriptionElement.dataClass.id
          ? '[]'
          : '';
  ...
  return inscription.inscriptionElements.length > 1 ? `(${inscriptionText})` : inscriptionText;
}

export function getLabel(element) {
  const businessObject = getBusinessObject(element);
  if (businessObject.$type === 'ptn:Arc') {
