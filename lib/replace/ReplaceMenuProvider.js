...
export default class ReplaceMenuProvider {
  constructor(customElementFactory, popupMenu, modeling, moddle,
    rules, translate, moddleCopy) {
  ...
  _createArcEntries(target) {
    ...
    if (target.businessObject.variableType) {
      entries['replace-with-arc'] = self._createEntry(
        ...
        target,
        function() {
          self._modeling.updateProperty(target, 'variableType', undefined, { rerender: true });
        }
      );
    } else {
      entries['replace-with-variable-arc'] = self._createEntry(
        ...
        target,
        function() {
          self._modeling.updateProperty(target, 'variableType', 'something', { rerender: true });
        }
      );
    }
