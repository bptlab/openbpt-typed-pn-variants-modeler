...
export default function ResetSimulation(eventBus, tokenSimulationPalette) {
  ...
  this._init();
  eventBus.on(Events.FIRE_TRANSITION_EVENT, () => {
    domClasses(this._paletteEntry).remove("disabled");
  });
  eventBus.on(Events.TOGGLE_MODE_EVENT, (event) => {
  ...
  eventBus.on(Events.RESET_SIMULATION_EVENT, () => {
...
ResetSimulation.prototype.resetSimulation = function () {
  this._eventBus.fire(Events.RESET_SIMULATION_EVENT);
};
