...
export default function Simulator(
  ...
  eventBus.on(Events.RESET_SIMULATION_EVENT, HIGH_PRIORITY, (event) => {
    this.reset();
  });

  eventBus.on(Events.TOGGLE_MODE_EVENT, HIGH_PRIORITY, (event) => {
    ...
    if (active) {
      ...
      try {
        if (commandStack) {
          ...
          eventBus.fire({ type: Events.COMMANDSTACK_CHANGED });
        }
      } catch (e) {
