      ...
      elementRegistry.forEach((element) => {
        if (this.basemarking.has(element.id)) {
          ...
          this.basemarking.get(element.id).forEach((values) => {
            // createToken is a helper that itself executes a command; to include
            // the token creations in the multi-command we create token moddle
            // objects here and add element.updateProperties commands that append
            // them to the marking.
            ...
            commands.push({
              ...
              context: {
                element,
                properties: {
                  marking: [...(element.businessObject.marking || []), token],
                },
              },
            });
          });
