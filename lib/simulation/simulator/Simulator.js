...
export default function Simulator(eventBus, elementRegistry, moddle) {
  ...
  function createToken(place, labels, binding) {
    const values = labels.map((label, index) =>
      moddle.create(`${MODELER_PREFIX}:TokenValue`, {
...
Simulator.$inject = ["eventBus", "elementRegistry", "moddle"];
