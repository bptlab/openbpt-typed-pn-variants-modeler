...
export default function ContextPads(eventBus, elementRegistry, overlays) {
  ...
  eventBus.on(Events.TOGGLE_MODE_EVENT, LOW_PRIORITY, (context) => {
  ...
  eventBus.on(
    [Events.FIRE_TRANSITION_EVENT, Events.RESET_SIMULATION_EVENT, Events.TRANSITION_FIRED_EVENT],
    LOW_PRIORITY,
    (event) => {
  ...
  eventBus.on(Events.COMMANDSTACK_CHANGED, LOW_PRIORITY, () => {
...
ContextPads.prototype._updateElementContextPads = function (element) {
  ...
  if (fireBtn) {
    domEvent.bind(fireBtn, "click", (event) => {
      ...
      this._eventBus.fire(Events.FIRE_TRANSITION_EVENT, { element });
    });
  }
  ...
  if (fastBtn) {
    domEvent.bind(fastBtn, "click", (event) => {
      ...
      this._eventBus.fire(Events.FIRE_TRANSITION_EVENT, { element, random: true });
    });
  }
