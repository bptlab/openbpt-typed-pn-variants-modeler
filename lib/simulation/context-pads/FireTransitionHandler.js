...
import { FIRE_TRANSITION_EVENT, TRANSITION_FIRED_EVENT } from "../EventHelper";
...
export class FireTransitionHandler extends CommandInterceptor {
  ...
  async fireTransition(transition) {
    ...
    const findMatchingToken = (place, inscriptionElements, binding) => {
      const marking = place.businessObject.marking || [];
          ...
          if (tokenToRemove) {
            commands.push({
              ...
              context: {
                element: source,
                properties: {
                  marking: (source.businessObject.marking || []).filter(
                    (t) => t !== tokenToRemove
                  ),
                },
              },
              ...
              properties: {
                marking: [
                  ...(target.businessObject.marking ||
                    []) /* token placeholder - actual token will be created by createToken wrapper */,
                ],
              },
            },
          });
  ...
  removeTokenFromPlaceForBinding(place, inscriptionElements, binding) {
    const marking = place.businessObject.marking || [];
    ...
    // Build expected value map: dataClass.id -> value for this place
    const expected = {};
    inscriptionElements.forEach((el, idx) => {
      const label = el.variableName;
      const dataClass = place.businessObject.color[idx];
      if (!dataClass) return;
      const value = el.isGenerated ? binding[label] : binding[label];
      // allow undefined -> skip
      if (value !== undefined) expected[dataClass.id] = value;
    });

    const matches = marking
      .map((token) => {
        const tokenMap = {};
        (token.values || []).forEach((v) => {
          if (v && v.dataClass && v.dataClass.id) {
        ...
        // compare expected keys only
        const allMatch = Object.keys(expected).every((k) => {
          // compare as strings for robustness
          return String(tokenMap[k]) === String(expected[k]);
        });
        return allMatch ? token : null;
      })
      .filter(Boolean);

    if (matches.length > 0) {
      // remove only the first matching token
      removeTokenFromPlace(place, matches[0], this.commandStack);
    }
  }
