...
export class FireTransitionHandler extends CommandInterceptor {
  ...
  async triggerTransition(transition) {
      ...
      <div id="bts-fire-overlay" class="bts-fire-overlay">
        <div class="bts-fire-content">
          ...
          <div class="bts-fire-footer">
            ...
            <button class="bts-fire-fast">Fast fire</button>
    ...
    // layout is handled by CSS; ensure fields container has the class (adapter may render differently)
    fieldsContainer.classList.add("bts-fire-fields");
    ...
    // ensure each direct child is a row (adapter may output rows differently)
    Array.from(fieldsContainer.children).forEach((child) => {
      if (child && child.classList) child.classList.add("bts-fire-row");
      const label =
        child && child.querySelector ? child.querySelector("label") : null;
      const sel =
        child && child.querySelector ? child.querySelector("select") : null;
      if (sel && sel.classList) sel.classList.add("bts-fire-field-select");
    });

    // compute required left column width from label sizes now that overlay is in DOM
    try {
      const labels = Array.from(fieldsContainer.querySelectorAll("label"));
      const max = labels.reduce((m, l) => {
        const w =
      ...
      const minW = Math.ceil(max) + 8; // small padding
      labels.forEach((l) => {
        if (l && l.style) {
          l.style.minWidth = `${minW}px`;
          l.style.margin = "0";
        }
      });
    } catch (e) {
    ...
    if (selectNodes.length > 0) {
      if (validBindings.length === 1) {
        ...
        selectNodes.forEach((s) => {
          const name =
            s.previousElementSibling && s.previousElementSibling.textContent
              ...
              : s.getAttribute("data-var");
          const val =
            single[name] !== undefined && single[name] !== null
              ? String(single[name])
              : "";
          if (val === "") {
            s.value = "";
          ...
          // ensure option exists; if not, append it
        ...
        // apply filters based on this initial selection
        updateFilters();
      }
    }
    ...
    // get buttons and wire them up; confirmBtn is declared above so key handler can check its state
    ...
    // implement confirm button enable/disable: only enabled when all selects have a non-empty value
    updateConfirmEnabled = () => {
    ...
    // initialize confirm button state now that updater and button are available
    updateConfirmEnabled();
