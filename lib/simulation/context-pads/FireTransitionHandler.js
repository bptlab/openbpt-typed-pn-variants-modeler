...
import { getValidInputBindings } from "../bindingUtilsNew";
...
export class FireTransitionHandler extends CommandInterceptor {
  ...
  async fireTransition(contextOrElement) {
     ...
     *
     * @param {Object[]} binding -  An array of binding objects mapping variableName -> value.
     */
    ...
    if (randomRequest) {
      if (validBindings && validBindings.length > 0) {
        ...
        for (const [dataClassKey, values] of Object.entries(binding)) {
          ...
          const dataClassId = dataClassKey.substring(0, dataClassKey.indexOf(":"));
          const dataClassAlias = dataClassKey.substring(dataClassKey.indexOf(":") + 1, dataClassKey.lastIndexOf(":"));
          const dataClassIsVariable = dataClassKey.substring(dataClassKey.lastIndexOf(":") + 1);
          if (dataClassIsVariable !== "true") {
            const value = values[Math.floor(Math.random() * values.length)];
            currently_chosen.push({[dataClassAlias]: value}); // TODO: Update this to use dataClassKey instead of label!
          }
          else{
