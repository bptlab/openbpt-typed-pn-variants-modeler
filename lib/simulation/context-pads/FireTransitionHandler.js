...
export class FireTransitionHandler extends CommandInterceptor {
    ...
    const executeBinding = (transitionEl, chosenBinding) => {
      ...
      if (outgoing) {
        for (const o of outgoing) {
          ...
          const initialTokenValuesList = (
            target.businessObject.color || []
          ).map((dataClass, idx) => {
            const mappedValues =
              this.getTokenValueMapFromBinding(
                target,
                inscriptionElements,
                dataClass,
                chosenBinding,
          ...
          const fullInitialTokenValuesList = this.specialCartesian(
            initialTokenValuesList,
          );
  ...
  generateValuesForBinding(outgoingArcs) {
    ...
    Object.keys(enriched).forEach((k) =>
      generatedBindings.push({ [k]: enriched[k] }),
    );
  ...
  specialCartesian(valuesList) {
    ...
    if (multi.length === 1) {
      ...
      return multiList.map((element) => {
        return [[element[0], ...singles.map((s) => s[0][0])]];
      });
    }
