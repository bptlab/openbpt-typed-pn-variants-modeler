...
export class FireTransitionHandler extends CommandInterceptor {
  constructor(
    ...
    eventBus.on(Events.FIRE_TRANSITION_EVENT, (context) =>
  ...
  async fireTransition(contextOrElement) {
    ...
    const executeBinding = (transitionEl, chosenBinding) => {
      ...
      if (outgoing) {
        for (const o of outgoing) {
          ...
          commands.push(buildAddTokenCommand(target, token));
        }
      }
      ...
      try {
        if (finalCommands.length > 0) {
          ...
          this._eventBus.fire(Events.TRANSITION_FIRED_EVENT, {
    ...
    try {
      this._eventBus.on &&
        this._eventBus.on(Events.TOGGLE_MODE_EVENT, toggleModeHandler);
    } catch (e) {
    ...
    const cleanup = () => {
      ...
      try {
        this._eventBus.off &&
          this._eventBus.off(Events.TOGGLE_MODE_EVENT, toggleModeHandler);
      } catch (e) {
        // ignore
      }
    };
    ...
    const applyBinding = (chosenBinding) => {
      ...
      try {
        if (finalCommands.length > 0) {
          ...
          this._eventBus.fire(Events.TRANSITION_FIRED_EVENT, { transition });
        }
      } catch (e) {
        console.warn("Failed executing fire transition commands", e);
