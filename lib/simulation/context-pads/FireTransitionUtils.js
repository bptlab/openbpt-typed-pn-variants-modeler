...
export function findMatchingToken(place, arcElements, variableType, binding) {
  ...
  const variableDataClass = {id: variableType?.id, alias: variableType?.alias};
  ...
  arcElements.forEach((el, i) => {
    ...
    const dataClassKey = `${color.id}:${color.alias}:${color.id === variableDataClass?.id && color.alias === variableDataClass?.alias}`;
  ...
  console.log(`Found ${matches.length} matching tokens in place ${place.id}: ${JSON.stringify(matches)}.`);
...
export function getTokenValueMapFromBinding(
  ...
  inscriptionElements.forEach((el) => {
    ...
    let values;
    if (el.isGenerated) {
      const value = generatedBinding[variableName]; // This is okay, as only non-variable data classes can be generated 
