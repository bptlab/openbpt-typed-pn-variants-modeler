...
export default function ToggleSimulationMode(
  ...
  eventBus.on(Events.IMPORT_PARSE_START, () => {
    if (this._active) {
      this.toggleMode(false);

      eventBus.once(Events.IMPORT_DONE, () => {
...
ToggleSimulationMode.prototype.toggleMode = function (active = !this._active) {
  ...
  if (active) {
    this._container.innerHTML = `Simulation Mode <span class="ptn-toggle"><span class="pn-icon-toggle-on"></span></span>`;

    domClasses(this._palette).add("hidden");
    domClasses(this._canvasParent).add("simulation");
  } else {
    this._container.innerHTML = `Simulation Mode <span class="ptn-toggle"><span class="pn-icon-toggle-off"></span></span>`;
  ...
  this._eventBus.fire(Events.TOGGLE_MODE_EVENT, {
    active,
  });
