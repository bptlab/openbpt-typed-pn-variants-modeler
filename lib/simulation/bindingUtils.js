...
export function getValidInputBindingsBasic(transition) {
  ...
  const singleArcBindings = arcBindings.filter((arcBindings) => arcBindings.arcType === "non-variable").map(ab => ab.binding);
  const variableArcBindings = arcBindings.filter((arcBindings) => arcBindings.arcType === "variable").map(ab => ab.binding);
