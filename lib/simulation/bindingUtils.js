...
export function getValidInputBindings(transition) {
  ...
  function isBlockedByInhibitor(binding) {
    for (const arc of [...incomingArcs, ...outgoingArcs]) {
      ...
      if (arc in incomingArcs) {
        console.log("Checking inhibitor arc in incoming arcs");
        const place = arc.source || null;
        if (!place || !place.businessObject) continue;
      } else {
        console.log(arc.target);
        const place = arc.target || null;
        if (!place || !place.businessObject) continue;
      }

      console.log("Checking inhibitor arc against place", place);
      ...
      for (const token of marking) {
        ...
        if (allMatch) { console.log("Inhibitor arc blocks binding"); return true; }
      }
    }
    console.log("Inhibitor arcs do not block binding");
    return false;
  }
