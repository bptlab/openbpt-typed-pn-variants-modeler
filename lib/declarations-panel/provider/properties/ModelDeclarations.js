...
export function ModelDeclarationsProps(props) {
  ...
  const simulationModeService = injector.get('simulationModeService');
  const readOnly = simulationModeService.isActive();
  ...
  const listItems = existingDeclarations.map((declaration) => {
    ...
    if (!readOnly) {
      item.remove = removeFactory({ element, declaration, commandStack, elementRegistry });
    }
...
function NameProperty(props) {
  ...
  const setValue = (value) => {
    ...
    commandStack.execute('element.updateProperties', {
      element,
      properties: {
        declarations: element.businessObject.declarations
      }
    });
  }
...
function addFactory({ element, customElementFactory, commandStack }) {
  return function(event) {
    ...
    const newDeclaration = createElement(`${MODELER_PREFIX}:DataClass`, { name: "NewClass", alias: "NewClass", valueType: "ID" }, element, customElementFactory);
...
function createElement(type, attrs, parent, customElementFactory) {
  const newElement = customElementFactory.create(type, attrs);
  if (parent) {
