...
export function getArcColor(arc) {
  const source = getBusinessObject(arc).source;
  const target = getBusinessObject(arc).target;
...
// Return a command descriptor that adds a token to a place's marking.
export function buildAddTokenCommand(place, token) {
  const newMarking = [...(place.businessObject.marking || []), token];
  return {
...
// Return a command descriptor that removes a token from a place's marking.
export function buildRemoveTokenCommand(place, token) {
  const newMarking = without(place.businessObject.marking || [], token);
  return {
    cmd: "element.updateProperties",
    context: {
      element: place,
