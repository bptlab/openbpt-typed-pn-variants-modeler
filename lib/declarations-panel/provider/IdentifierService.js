import { add } from "diagram-js/lib/util/Elements";
...
export default function IdentifierService(elementRegistry) {
  ...
  this.reserveIdentifier = (dataClass, dataClassKey) => {
    const classId = dataClass.id || dataClass.name;
    const variableName = dataClassKey.split(":")[1] || dataClass.alias;

    let addition = "";
    addition = variableName.startsWith(dataClass.alias)
      ? variableName.slice(dataClass.alias.length)
      : variableName;
    const baseName = dataClass.name + addition;

    // seed next counter if missing
    if (this._nextCounterByClass[dataClassKey] == null) {
      // compute max existing numeric suffix for this class
      const existing = Array.from(this._collectExistingIds()).filter((id) =>
        new RegExp(`^${_escapeRegex(baseName)}_(\\\\d+)$`).test(id),
      );
      let maxId = 0;
      existing.forEach((id) => {
        const match = id.match(
          new RegExp(`^${_escapeRegex(baseName)}_(\\\\d+)$`),
        );
        if (match) {
      ...
      this._nextCounterByClass[dataClassKey] = maxId + 1;
    }

    let candidate = null;
    // loop until we find a candidate not present in current ids or reservations
    do {
      candidate = `${baseName}_${this._nextCounterByClass[dataClassKey]++}`;
    } while (
  ...
  this.generateUniqueId = (dataClass, variableName) => {
    const token = this.reserveIdentifier(dataClass, variableName);
    this.commitReservation(token);
    return token.id;
  };
}
