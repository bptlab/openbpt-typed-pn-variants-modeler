...
export default class CustomContextPadProvider {
  constructor(connect, contextPad, modeling, elementFactory, create, autoPlace, popupMenu, canvas) {
  ...
  getContextPadEntries(element) {
    ...
    if (is(element, `${MODELER_PREFIX}:Arc`) && !isLabel(element)) {
      if (!popupMenu.isEmpty(element, 'tpn-replace')) {
        assign(actions, {
          'replace': {
            group: 'row_3',
            ...
            action: {
              click: function(event, element) {
                ...
                popupMenu.open(element, 'tpn-replace', position, {
                  title: 'Change element',
                  width: 300,
                  search: true
                });
