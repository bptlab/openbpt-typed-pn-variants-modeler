    ...
    if (is(element, `${MODELER_PREFIX}:Arc`) && !isLabel(element)) {
      assign(actions, {
        "toggle-inhibitor-arc": {
          group: "row_0",
          className: element.businessObject.isInhibitorArc ? "bpmn-icon-connection" : "pn-icon-inhibitor-arc",
          title: element.businessObject.isInhibitorArc ? "Change to regular arc" : "Change to inhibitor arc",
          action: {
